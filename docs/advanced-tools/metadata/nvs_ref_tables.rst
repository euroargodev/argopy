.. currentmodule:: argopy.reference
...
- :ref:`Reference **values** <argoreferencevalue>`_, aka NVS "concept", eg: value "AANDERAA_OPTODE_3930" is one documented and possible value for the "SENSOR_MODEL" parameter in netcdf files,
- :ref:`Reference **tables** <argoreferencetable>`_, aka NVS "vocabulary", eg: table "R27" documenting the list of possible values for the "SENSOR_MODEL" parameter in netcdf files, like "AANDERAA_OPTODE_3930",
- :ref:`Reference **value relationships** <argoreferencemapping>`_, aka a NVS "mapping", eg: values from the "R27/SENSOR_MODEL" table like "AANDERAA_OPTODE" are _related_ to values from the "R25/SENSOR" table like "OPTODE_DOXY" and are _narrower_ concept of values from the "R26/SENSOR_MAKER" table like "AANDERAA".

**Argopy** provides specific utility classes for each of these objects that are detailled below.
...
An Argo Reference Value represents one possible and documented value for an Argo parameter. For example, ``AANDERAA_OPTODE_3835`` is an Argo Reference Value for the ``SENSOR_MODEL`` parameter. All possible values for this parameter are listed in the **Argo reference table 27** for "Argo sensor models" (:ref:`see below <argoreferencetable>_`).
...
For instance, the BBP470 definition reads: ``Particle backscattering at 470 nm wavelength, reported by ECO3 sensor. Local_Attributes:{long_name:Particle backscattering at 470 nanometers; standard_name:-; units:m-1; valid_min:-; valid_max:-; fill_value:99999.f}. Properties:{category:b; data_type:float}``. **Argopy** automaticaly infer those extra-attributes like ``Local_Attributes`` and ``Properties`` from this example.
...
For example, the vocabulary for "Argo sensor models" corresponds to the Argo reference table 27 ("R27") and is used to document possible values of the ``SENSOR_MODEL`` parameter in NetCDF files. All possible values for this parameter are instances of :class:`ArgoReferenceValue` (:ref:`see above <argoreferencevalue>_`).
...
Since :class:`ArgoReferenceTable` represents a list of distinct concepts, **Argopy** uses an dictionary-like indexing where _keys_ are the table values name:
...
**A class to work with Argo Reference Value Relationships, a.k.a. a NVS "mapping".**
...
The relationship is also called a _predicate_. The `AVTT documentation
...
.. autoclass:: ArgoReferenceMapping
    :noindex:
...
Since a mapping describes the relationship between an _subject_ and an _object_, you can create a :class:`ArgoReferenceMapping` by providing them in this order:
...
Here, ``PLATFORM_MAKER`` is the _subject_ and ``PLATFORM_TYPE`` the _object_.
...
.. note::
    ...
    **New API (but not backward compatible):**
