...
The ocean is a key component of the Earth climate system. It therefore needs continuous real-time monitoring to help scientists 
...
Moreover, since the Argo community focuses on delivering a curated dataset for science, it does not provide open source software tools 
for its user base. Software packages exist for Argo data operators so that they can decode and quality control the data [e.g. @scoop],
but none are available for scientists who therefore must develop their own machinery to download and manipulate the data.
