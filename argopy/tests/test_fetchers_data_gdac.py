...
#
# For a Dask client, see the dedicated test_fetchers_dask_cluster.py
#
# #todo The parallel fetching integration is not tested with the GDAC data fetcher
#
# """
# PARALLEL_OPTS = [
#     {"parallel": "thread"},
...
def assert_fetcher(server, this_fetcher, cacheable=False):
    ...
    try:
        ...
        if sys.platform.startswith("win32") or sys.platform.startswith("cygwin"):
            pytest.xfail(
                "Fails because of Windows permissions error that can't be fixed (e.g. https://github.com/python/cpython/issues/66305)"
            )
        else:
            raise
...
for host in HOSTS:
    for mode in USER_MODES:
        for ap in ACCESS_POINTS:
            VALID_ACCESS_POINTS.append(
                {"host": host, "ds": "phy", "mode": mode, "access_point": ap}
            )
            VALID_ACCESS_POINTS_IDS.append(
                "host='%s', ds='%s', mode='%s', %s"
                % (gdac_shortname(host), "phy", mode, ap)
            )
...
# for host in HOSTS:
#     for mode in USER_MODES:
#         for ap in PARALLEL_ACCESS_POINTS:
#             for opts in PARALLEL_OPTS:
...
#                     f"host='{gdac_shortname(host)}', ds='phy', mode='{mode}', parallel='{opts}', {ap}"
#                 )
#
...
class TestBackend:
    ...
    @pytest.mark.parametrize(
        "fetcher", VALID_ACCESS_POINTS, indirect=True, ids=VALID_ACCESS_POINTS_IDS
    )
