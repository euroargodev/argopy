...
class Test_NVS_Offline():
    ...
    @pytest.mark.parametrize("rtid", RTID_TESTED, indirect=False, ids=[f"rtid='{x}'" for x in RTID_TESTED])
    def test_vocabulary2uri(self, nvs, rtid):
        assert is_offline_urivalid(nvs._vocabulary2uri(rtid))

    @pytest.mark.parametrize("fmt", ['xml', 'turtle'], indirect=False, ids=[f"fmt='{x}'" for x in ['xml', 'turtle']])
    @pytest.mark.parametrize("rtid", RTID_TESTED, indirect=False, ids=[f"rtid='{x}'" for x in RTID_TESTED])
    def test_vocabulary2uri_invalidformat(self, nvs, rtid, fmt):
        with pytest.raises(ValueError):
            nvs._vocabulary2uri(rtid, fmt=fmt)

    @pytest.mark.parametrize("fmt", ['json'], indirect=False, ids=[f"fmt='{x}'" for x in ['json']])
    @pytest.mark.parametrize("rtid", ['R27', None], indirect=False, ids=[f"rtid='{x}'" for x in ['R27', None]])
    @pytest.mark.parametrize("conceptid", ['AANDERAA_OPTODE_3835'], indirect=False, ids=[f"conceptid='{x}'" for x in ['AANDERAA_OPTODE_3835']])
    ...
    @pytest.mark.parametrize("fmt", ['json'], indirect=False, ids=[f"fmt='{x}'" for x in ['json']])
    @pytest.mark.parametrize("rtid", RTID_TESTED, indirect=False, ids=[f"rtid='{x}'" for x in RTID_TESTED])
    @pytest.mark.parametrize("conceptid", CONCEPTID_TESTED, indirect=False, ids=[f"conceptid='{x}'" for x in CONCEPTID_TESTED])
...
class Test_NVS_Online():
    ...
    @pytest.mark.parametrize("fmt", FMT_TESTED, indirect=False, ids=[f"fmt='{x}'" for x in FMT_TESTED])
    @pytest.mark.parametrize("rtid", RTID_TESTED, indirect=False, ids=[f"rtid='{x}'" for x in RTID_TESTED])
